...
export default async (data: { body: { logoType: string; file: string } }) => {
  ...
  try {
    ...
    // Update settings in database with the new logo URL
    if (successCount > 0) {
      const cacheManager = CacheManager.getInstance();
      await cacheManager.updateSetting(logoType, logoUrl, true); // Update in cache and DB
      console.log(`[LOGO-SETTINGS-UPDATE] Updated setting ${logoType} = ${logoUrl}`);
    }
