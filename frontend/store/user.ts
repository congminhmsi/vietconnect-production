  ...
  return {
    ...
    login: async (email: string, password: string) => {
      ...
      try {
        ...
        // Set authentication cookies from response if provided
        if (data && data.cookies) {
          if (typeof window !== "undefined") {
            try {
              // Set cookies in browser
              if (data.cookies.accessToken) {
                document.cookie = `accessToken=${data.cookies.accessToken}; path=/; max-age=${7 * 24 * 60 * 60}; SameSite=Lax; Secure`;
              }
              if (data.cookies.sessionId) {
                document.cookie = `sessionId=${data.cookies.sessionId}; path=/; max-age=${7 * 24 * 60 * 60}; SameSite=Lax; Secure`;
              }
              if (data.cookies.csrfToken) {
                document.cookie = `csrfToken=${data.cookies.csrfToken}; path=/; max-age=${7 * 24 * 60 * 60}; SameSite=Lax; Secure`;
              }
              console.log("Authentication cookies set successfully");
            } catch (cookieError) {
              console.error("Error setting authentication cookies:", cookieError);
            }
          }
        }
