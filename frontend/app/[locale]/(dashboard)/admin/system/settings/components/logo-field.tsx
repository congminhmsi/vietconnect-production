...
export function LogoField({ field, value, onChange }: LogoFieldProps) {
  ...
  // Get the logo URL to display - prioritize the value from settings, fallback to mapped file path
  const getDisplayLogoUrl = () => {
    // First, try to use the value from settings (updated after upload)
    if (value && value.startsWith('/img/logo/')) {
      // During SSR, don't add timestamp to prevent hydration mismatch
      if (!mounted) {
        return `${value}?v=${logoVersion}`;
      }
      // Add cache busting only after mounting
      return `${value}?v=${logoVersion}&t=${Date.now()}`;
    }

    // Fallback to mapped file path if no value is set
