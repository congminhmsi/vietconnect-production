...
if (require("fs").existsSync(envPath)) {
  ...
  console.warn("Frontend: No .env file found");
}
...
const nextConfig = {
  ...
  webpack: (config, { dev, isServer }) => {
    ...
    // Fixes npm packages that depend on `fs` module and @reown packages
    if (!isServer) {
      config.resolve.fallback = {
        ...
        path: false, // Override path-browserify for @reown compatibility
        encoding: require.resolve('encoding'),
        buffer: false, // For @reown packages
      };
    }
